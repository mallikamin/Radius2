...
export default function VectorMap() {
  ...
  // NOTE: annosRef/plotsRef/inventoryRef REMOVED — replaced by deterministic refs inside useVectorState
  // Use vectorState.getLatestAnnos() / getLatestPlots() / getLatestInventory() for save operations
  ...
  const loadedMapTypeRef = useRef('master'); // Track loaded project's mapType
  const isAutoGeneratedRef = useRef(false); // Track if loaded project is auto-generated
  ...
  // Effect-synced refs REMOVED: they fired AFTER render (always one tick stale).
  // useVectorState now syncs refs INSIDE every mutator (getLatestAnnos/Plots/Inventory).
  ...
  const handleSaveToDatabase = async () => {
    ...
    // GUARD: Block saving auto-generated maps (status/customer/auto_master)
    if (isAutoGeneratedRef.current) {
      alert(`Cannot save: this is an auto-generated ${loadedMapTypeRef.current} map.\nEdits to auto-generated maps are overwritten on next sync.\nOpen the master project to make changes.`);
      return;
    }
    ...
    try {
      ...
      // Use deterministic refs from useVectorState (synced inside every mutator — never stale)
      const annosToSave = vectorState.getLatestAnnos();
      const plotsToSave = vectorState.getLatestPlots();
      const inventoryToSave = vectorState.getLatestInventory();
      ...
      const vectorMetadata = {
        ...
        plotRotations: vectorState.getLatestPlotRotations() || {},
      ...
      console.log('SAVE OK — project:', savedProjectId, '| PDF sent:', !!projectData.map_pdf_base64);
      alert(`Saved! Project ID: ...${String(savedProjectId).slice(-8)}`);
  ...
  const handleLoadProjectFromDB = async (projectId) => {
    ...
    try {
      ...
      console.log('LOAD OK — project:', projectId, '(last8:', String(projectId).slice(-8), ') |', data.projectName, '|', data.plots?.length || 0, 'plots,', data.annos?.length || 0, 'annos, PDF:', !!data.pdfBase64);
      ...
      loadedMapTypeRef.current = data.mapType || 'master';
      isAutoGeneratedRef.current = data.isAutoGenerated || false;
      pdfChangedRef.current = false; // Fresh load — PDF not changed
      console.log(`LOAD IDENTITY — id:...${String(projectId).slice(-8)} mapType:${loadedMapTypeRef.current} isAuto:${isAutoGeneratedRef.current}`);
      ...
      // loadProjectData already syncs deterministic refs (_annosRef, _plotsRef, _inventoryRef)
      // inside useVectorState, so no recovery/force-re-render needed here.
