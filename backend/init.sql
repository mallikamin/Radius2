...
-- Sequence for file_id generation
CREATE SEQUENCE IF NOT EXISTS media_file_id_seq START 1;

-- Function to generate file_id
...
-- Trigger for file_id generation
DROP TRIGGER IF EXISTS trg_media_file_id ON media_files;
CREATE TRIGGER trg_media_file_id 
    BEFORE INSERT ON media_files 
    FOR EACH ROW 
    EXECUTE FUNCTION generate_media_file_id();
