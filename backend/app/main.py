...
def get_vector_project(
    ...
    try:
        ...
        # Detailed debug: project identity + mapType + rotation
        map_type = project.vector_metadata.get('mapType', 'master') if project.vector_metadata else 'master'
        is_auto = project.vector_metadata.get('isAutoGenerated', False) if project.vector_metadata else False
        print(f"GET [{str(project.id)[-8:]}] name={project.name} mapType={map_type} isAuto={is_auto} plots={len(plots)}")
        ...
        if project.vector_metadata and 'annos' in project.vector_metadata:
            ...
            if isinstance(metadata_annos, list):
                ...
                # Debug: Log ALL annotation IDs + rotation on load
                print(f"GET [{str(project.id)[-8:]}] annos: {[(str(a.get('id','?'))[-8:], a.get('note',''), a.get('rotation', 0)) for a in annos[:10]]}")
...
def update_vector_project(
    ...
    if vector_metadata is not None:
        ...
        if parsed_metadata:
            ...
            if isinstance(annos_in_save, list) and len(annos_in_save) > 0:
                ...
                rotated_annos = [a for a in annos_in_save if a.get('rotation', 0) != 0]
                if rotated_annos:
                    print(f"DEBUG UPDATE: Annotations with rotation: {[(a.get('note',''), a.get('rotation')) for a in rotated_annos]}")
        project.vector_metadata = parsed_metadata
        # CRITICAL: flag_modified ensures SQLAlchemy includes JSONB column in UPDATE statement
        from sqlalchemy.orm.attributes import flag_modified
        flag_modified(project, 'vector_metadata')
    ...
    # POST-COMMIT VERIFICATION: confirm rotation persisted in DB
    if project.vector_metadata and 'annos' in project.vector_metadata:
        persisted_annos = project.vector_metadata.get('annos', [])
        persisted_rotated = [a for a in persisted_annos if isinstance(a, dict) and a.get('rotation', 0) != 0]
        print(f"VERIFY POST-COMMIT [{str(project.id)[-8:]}]: {len(persisted_annos)} annos, {len(persisted_rotated)} with rotation: {[(a.get('note',''), a.get('rotation')) for a in persisted_rotated[:5]]}")
